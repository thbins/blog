...
**서비스 메시**란 분산된 마이크로서비스 간의 통신을 **투명하게 제어·보안·관찰**하기 위한 **인프라 레벨의 네트워크 계층(추가 레이어)** 입니다.

> 즉, 쿠버네티스가 Pod를 관리하는 것에 집중했다면, 서비스 메시는 Pod 간의 통신을 관리하는 것에 집중한다는 것입니다.
...
마이크로서비스 아키텍처는 수많은 서비스(Pod)로 구성될 수 있으며, 이 Pod들 간의 호출 관계는 복잡한 **N:N 통신 그래프**를 형성합니다.
그러나 이러한 구조를 쿠버네티스 자체 기능만으로 지원하기에는 아래 표와 같은 한계가 존재합니다.
...
서비스 메시는 일반적으로 **Control Plane**과 **Data Plane**의 두 계층으로 구성됩니다.
...
Pod 간 통신은 위와 같이 **모두 Envoy Proxy를 거쳐서 이루어지며**, 이 프록시는 중간에서 트래픽을 **모니터링·제어·암호화**합니다.
